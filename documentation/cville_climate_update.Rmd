...
<details><summary>Click to see Equity Center Data Team contributors</summary><br/>
...
<p style="text-align: center;">[Data Collection Process](#process) | [Data Collections](#collection) | [Examples](#examples)</p>
...
<img src="../images/Data Process.png" width="500" align="center">
...
lead %>% 
  ...
  ggplot(aes(x = reorder(tract_fip, percentburdened), y = percentburdened, fill = percentburdened)) +
    geom_col() + scale_fill_viridis(option="plasma", guide="none") +
...
```{r}
lead %>% 
  filter(county=="Albemarle" & tract_fip != "010903") %>%
   ggplot(aes(x = reorder(tract_fip, percentburdened), y = percentburdened, fill = percentburdened)) +
...
```{r}
# load data
lodesrescommute <- read_csv("../data-csv/lodes_residentcommute_cville_blkgp.csv")
lodesworkpatterns <- read_csv("../data-csv/lodes_workerscommutepatterns_cvlcounty.csv")

# read shapefile
shape <- readRDS("../data-csv/cville_blkgps.RDS")
...
lodesworkpatterns %>% 
  ...
  dplyr::filter(commuterstoRegion > 1000) %>% 
  ...
  dplyr::mutate(name = fct_reorder(name, commuterstoRegion)) %>% 
  ggplot(aes(x = commuterstoRegion, y = name, color = commuterstoRegion)) + 
  geom_segment(aes(x = 0, y = name, xend = commuterstoRegion, yend = name)) +
        geom_point(size = 7) + 
...
# load data
stats <- read_csv("../data-csv/landsat8_cville_city.csv")
scene <- stats %>% 
...
#### Average Maximum Temperatures, Annually
This is the average of the maximum temperature recorded across all months for each year.

```{r}
# load data
noaa <- read_csv("../data-csv/noaa_cville_county.csv")
cville_sf <- readRDS("../data-csv/cville_counties.RDS")
cville_sf <- cville_sf %>% rename(county = COUNTYFP)

cville_noaa <- left_join(cville_sf, noaa, by ="county")

cville_noaa <- cville_noaa %>% 
  filter_at(vars(Avg_Tempmin:Decpcp),all_vars(!is.na(.))) %>% 
